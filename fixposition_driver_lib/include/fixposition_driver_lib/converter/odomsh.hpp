/**
 *  @file
 *  @brief Declaration of OdomshConverter
 *
 * \verbatim
 *  ___    ___
 *  \  \  /  /
...
#ifndef __FIXPOSITION_DRIVER_LIB_CONVERTER_ODOMSH__
#define __FIXPOSITION_DRIVER_LIB_CONVERTER_ODOMSH__
...
class OdomshConverter : public BaseAsciiConverter {
   public:
    /**
     * @brief Data for Messages published from the ODOMSH msg
     *
     */
    struct Msgs {
    ...
    using OdomshObserver = std::function<void(const Msgs&)>;

    /**
     * @brief Construct a new Fixposition Msg Converter object
     *
     */
    OdomshConverter() : BaseAsciiConverter() {}

    ~OdomshConverter() = default;

    /**
     * @brief Comma Delimited FP,ODOMSH message, convert to Data structs and if available,
     * call observers
     * Example:
     ...
     * @param[in] ob
     */
    void AddObserver(OdomshObserver ob) { obs_.push_back(ob); }
   ...
   private:
    ...
    Msgs msgs_;
    std::vector<OdomshObserver> obs_;
};

}  // namespace fixposition
#endif  // __FIXPOSITION_DRIVER_LIB_CONVERTER_ODOMSH__
